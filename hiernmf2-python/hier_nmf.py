...
def hier8_neat(X, k, tol=1e-4, maxiter=10000, trial_allowance=3, unbalanced=0.05):
    ...
    children = {}
    leaf_label = np.ones(n, dtype=np.int64) # doc labels (leaf nodes)
    Ws = np.zeros((m, 2 * (k - 1)))
    Hs = np.zeros((2 * (k - 1), n))
    W_buffer = np.empty(2 * (k - 1), dtype=object)
    H_buffer = np.empty(2 * (k - 1), dtype=object)
    ...
    for i in range(0, k - 1):
        ...
        if i == 0:
            ...
            cur_priority = temp_priority[split_node]
            if initFlag:
                initFlag = False
                old_priority = temp_priority[split_node]
                cur_priority = old_priority
            if old_priority - cur_priority > 0.02:
                mergeFlag = False
            else:
                mergeFlag = True
            old_priority = cur_priority
            print(cur_priority)
            ...
            split_subset = np.where(leaf_label == split_node)[0]
            new_nodes = [result_used, result_used + 1]
            # tree[0, split_node] = new_nodes[0]
            # tree[1, split_node] = new_nodes[1]
        parent[new_nodes[0]] = split_node
        parent[new_nodes[1]] = split_node
        children[split_node].append(new_nodes[0])
        children[split_node].append(new_nodes[1])
        children[new_nodes[0]] = []
        children[new_nodes[1]] = []

        # Split further or merge to current layer
        if mergeFlag:
            parent[new_nodes[0]] = parent[split_node]
            parent[new_nodes[1]] = parent[split_node]
            children[parent[split_node]].append(new_nodes[0])
            children[parent[split_node]].append(new_nodes[1])
            del children[split_node]
            children[parent[split_node]].remove(split_node)
        ...
        leaf_label[split_subset[np.where(cluster_subset == 0)[0]]] = new_nodes[0]
        leaf_label[split_subset[np.where(cluster_subset == 1)[0]]] = new_nodes[1]
        Ws[:, new_nodes[0]] = W[:, 0]
        Ws[:, new_nodes[1]] = W[:, 1]
        Hs[new_nodes[0], :] = H[0, :]
        Hs[new_nodes[1], :] = H[1, :]
        ...
        subset = np.where(leaf_label == new_nodes[0])[0]
        subset_new, W_buffer_one, H_buffer_one, priority_one = trial_split(trial_allowance, unbalanced, min_priority, X, subset, W[:, 0], tol, maxiter)
        # clusters[new_nodes[0]] = subset
        subset_diff = np.setdiff1d(subset, subset_new)
        leaf_label[subset_diff] = -1
        ...
        subset = np.where(leaf_label == new_nodes[1])[0]
        subset_new, W_buffer_one, H_buffer_one, priority_one = trial_split(trial_allowance, unbalanced, min_priority, X, subset, W[:, 1], tol, maxiter)
    ...
    return parent, children, leaf_label, is_leaf, timings, Ws, Hs, priorities
...
#        print(np.shape(X_subset))
        ...
        if len(np.unique(cluster_subset)) > 1:
            priority_one = compute_priority(W_parent, W_buffer_one, X[:,subset], cluster_subset)
        else:
            priority_one = -1
...
#@profile
def compute_priority(W_parent, W_child, X, cluster, score='NDCG_part'):
    if score == 'dc':
        priority = score_func(W_parent, W_child, X, cluster)
        return priority
    n = len(W_parent)
...
#@profile
def score_func(W_parent, W_child, X, cluster):
    # if sps.issparse(B):
    print(np.shape(X), np.shape(W_parent))
    hp = X.T.dot(W_parent)
    print(len(np.where(hp<=0)[0]))
    # score = np.linalg.norm(X - np.outer(W_parent, hp), ord='fro')
    # score = score - np.linalg.norm(X[:, cluster==0] - W_child[:, 0] * h1, ord='fro')
    # score = score - np.linalg.norm(X[:, cluster==1] - W_child[:, 1] * h2, ord='fro')
    return score
...
#    print '12', time.time() - t0; t0 = time.time()
    ...
    for iterNumber in range(0, maxiter):
        if np.abs(left[0, 0]) < eps or np.abs(left[1, 1]) < eps:
            grad = -1
...
def anls_entry_rank2_precompute(left, right, matrixH):
    ...
    # cosine_either = solve_either * values
...
def print_deliverable(parent, children, leaf_label, is_leaf, timings, Ws, Hs, priorities,A,dic,ids):
    from scipy import spatial
    with open('deliverable_words.tsv', 'w') as f1:
        f1.write('\t'.join(
            ['topic', 'sub_topic', 'word', 'conf_measure']) + '\n')
        W = Ws[:, is_leaf == 1]
        leaf_ids = np.where(is_leaf == 1)
        for i in range(len(dic)):
            w=W[i,:]
            max_id = np.argmax(w)
            top_topic = leaf_ids[0][max_id]

            topic_id = top_topic
            subtopic_id = -1
            while parent[topic_id] != -1:
                subtopic_id = topic_id
                topic_id = parent[topic_id]

            conf = w[max_id]/np.sum(w)
            f1.write('\t'.join([str(topic_id),
                                str(subtopic_id),
                                dic[i],
                                str(conf),
                                ]) + '\n')
    with open('deliverable_dataset.tsv','w') as f1:
        f1.write('\t'.join(['topic','sub_topic','dataset_id','conf_measure'])+'\n')
        for i in range(len(leaf_label)):
            topic_id = leaf_label[i]
            subtopic_id = -1
            while topic_id != -1:
                conf = 1 - spatial.distance.cosine(A[:,i].toarray(),Ws[:,topic_id])
                f1.write('\t'.join([str(topic_id),
                                    str(subtopic_id),
                                    ids[i],
                                    str(conf),
